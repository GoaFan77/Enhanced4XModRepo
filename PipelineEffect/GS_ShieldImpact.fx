...
GetPixelColor( float4 iColor, float2 iTexCoord, float iDist, float3 iPos, float3 iView)
{
	...
	float impactDistVertex = (3 * iDist) - 50; //Changes how far shield effect spreads. uses inter-vertex interpolation - less accurate but good over longer range
	...
	float dissipationTime = g_Time * 1.0;// Scale time
